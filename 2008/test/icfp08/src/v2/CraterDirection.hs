...
chooseStrtgHighest defaultAct strtgs = maximum all -- (trace (show all) all)
...
data AlienDanger = Low | High | ItIsAMonster
--                  0    1       2

-- madAllien :: Message -> AlienDanger
madAllien maxSpeed (Telemetry _ _ _ pos@(vpx, vpy) dir speedR objs ) = maximum $ 0 :  getAliens
    where getAliens = [ severity aSpeed $ cosAlien ax ay | (Martian (ax, ay) aDir aSpeed )<- objs]
          severity speedal angCos
              | (speedal > maxSpeed) && (angCos < ( cos $ toRadian 150)) = 3::Int
              | (speedal > speedR) && (angCos < ( cos $ toRadian 150)) = 2
              | otherwise = 0
          cosAlien ax ay = dotProduct (angleToVec dir) norm2Allien
              where dst2Alien = dist pos (ax, ay)
                    norm2Allien = (( (ax - vpx) / dst2Alien ) ,((ay - vpy ) / dst2Alien ))
madAllien _ _ = 0
...
steering init@(Init _ _ _ _ _ maxspeed _ _) mesg@(Telemetry _ _ _ _ _ speed objs)
    | ( madAllien maxspeed mesg ) > 1 = steerAcc steer
    | density == 0  = steerAcc steer
    | (density > 20 ) && ( speed > maxspeed * 0.40) = steerBrake steer
    | (density > 15 ) && ( speed > maxspeed * 0.50) = steerBrake steer
    | (density > 10 ) && ( speed > maxspeed * 0.60) = steerBrake steer
    | speed > (maxspeed * 0.95 ) = steerBrake steer
    | speed < (maxspeed * 0.06) = steerAcc steer -- keep the show moving
    | otherwise = steer
    where
      density = length objs
...
homingSteer init@(Init _ _ maxtime _ _ _ _ _) (Telemetry time _ turn vp@(vpx, vpy) dir _ objs) = Strategy steer' priority
 where myVec = angleToVec dir
       ...
       distanceToCenter = sqrt $ dotProduct vp vp
       dotP = dotProduct (angleToVec ( dir)) (-vpx / distanceToCenter, -vpy / distanceToCenter)
       absDotP =   dotP
       steer' | (distanceToCenter < 35 ) &&  (absDotP < (cos $ toRadian 40)) = steerBrake (steerBrake steer)
              | (distanceToCenter < 35 ) && (absDotP < (cos $ toRadian 30)) = steerBrake steer
              | otherwise = steer
